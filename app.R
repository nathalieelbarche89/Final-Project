  ...
  ")),
    ...
    tabItems(
      ...
      tabItem(tabName = "visualizations",
              ...
              fluidRow(
                column(width = 12,
                       div(style = "background-color: #E6F9E6; color: #05A65A; border: 2px solid #05A65A; border-radius: 10px; padding: 20px; margin-bottom: 20px;",
                           h3("Filter", style = "color: #05A65A; font-weight: bold;"),
                           selectInput("location_filter", "Filter by Location:",
                                       choices = c("All", unique(sustainability_data$Location)),
                                       selected = "All")
                       )
                )
              ),
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    if (input$location_filter == "All") {
      ...
      sustainability_data[sustainability_data$Location == input$location_filter, ]
    }
  })
  ...
  green_palette <- c("Urban" = "#31a354", "Suburban" = "#74c476", "Rural" = "#006d2c")
    output$energy_vs_home_size <- renderPlotly({
      plot_data <- filtered_data()
      p <- ggplot(plot_data, aes(x = HomeSize, y = MonthlyElectricityConsumption, color = Location)) +
  ...
  #Extra visualizations (added later)
